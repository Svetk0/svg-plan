...
// ========== Good Feature ==================
// =============  FUNCTION of cycle in cycle ==============
//const dataKeys = ["name", "universe", "alterego", "occupation", "friends", "superpowers", "url", "info"];
...
for (let i = 0; i < data.length; i++) {
    ...
    for (let j = 0; j < dataKeys.length; j++) {
        // console.log('dataKeys: ' + dataKeys[j]);
        //data[i].key = dataKeys[j];
        console.log(dataKeys[j] + ': ' + data[i][dataKeys[j]]);
    }
}
...
function createInfoCard_byCat(card) {
    const newCard = document.createElement("div");
    newCard.classList.add("item-properties");

    const divName = createCardTextElement_byCat(card.name, "name");
    const divUniverse = createCardTextElement_byCat(card.universe, "universe", "Вселенная: ");
    ...
    //Add Rating
    const divRating = createCardTextElement_byCat('', "rating");
    createRating(divRating);
...
function createRating(divContainer) {
    //const divContainer = createCardTextElement_byCat('', "rating");
    ...
    for (let j = 0; j < starCount; j++ ) {
        //let str = gfg(1);
        clickFuncs[j] = 'gfg(' + (j + 1) + ')';
        //console.log(clickFuncs[j]);
    }
    ...
    for (let i = 0; i < starCount; i++) {
        ...
        stars[i].textContent = '★';
        stars[i].onclick = clickFuncs[i];
        divContainer.appendChild(stars[i]);
    }
